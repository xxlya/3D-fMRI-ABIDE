...
class fMRIDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_blanket = []
        for i in range(50):
            ind = random.sample(range(0, self.T - self.nch + 1), 1)[0]  # sample index
            data_blanket.append(fMRI[ind:ind + self.nch, :])
        onebatch_x = np.stack(data_blanket)
        onebatch_y = np.repeat(self.csv[self.csv['SUB_ID']==id]['DX_GROUP'].values[0]%2, 50)
...
def get_loader(datadir, win_size, ID, T):
    ...
    # define dataloader
    data_set = fMRIDataset(datadir, win_size, ID, T)
